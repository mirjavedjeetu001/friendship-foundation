    ...
    <div class="py-6" x-data="electionForm()">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($election->status === 'paused')
                <div class="mb-4 p-4 bg-yellow-900/50 border border-yellow-700 text-yellow-400 rounded-lg flex items-center gap-2">
                    <span>⏸️</span>
                    <span>This election is paused. You can edit it now. Resume it when ready.</span>
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('admin.elections.update', $election) }}" class="space-y-6">
                ...
                <!-- Type Display (read-only for existing elections) -->
                <div class="bg-gray-800 rounded-lg p-6">
                    <h3 class="text-lg font-semibold text-white mb-4">Type</h3>
                    ...
                    <input type="hidden" name="type" value="{{ $election->type }}">
                </div>
                ...
                <div class="bg-gray-800 rounded-lg p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $election->title) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" name="title_bn" value="{{ old('title_bn', $election->title_bn) }}"
                                class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-white">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-300 mb-1">Description</label>
                            <textarea name="description" rows="3"
                                class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-white">{{ old('description', $election->description) }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="term_years"
                                class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-white">
                                <option value="1" {{ old('term_years', $election->term_years) == 1 ? 'selected' : '' }}>1 Year</option>
                                <option value="2" {{ old('term_years', $election->term_years) == 2 ? 'selected' : '' }}>2 Years</option>
                                <option value="3" {{ old('term_years', $election->term_years) == 3 ? 'selected' : '' }}>3 Years</option>
                            </select>
                        </div>
                        @endif
                    </div>
                ...
                <div class="bg-gray-800 rounded-lg p-6">
                    ...
                    <template x-for="(option, oIndex) in options" :key="oIndex">
                        <div class="flex items-center space-x-3 mb-3">
                            ...
                            <input type="text" name="options[]" x-model="options[oIndex]" required
                ...
                if ($election->type === 'election') {
                    foreach ($election->positions as $pos) {
                        $positionsData[] = [
                            ...
                            'name' => $pos->name,
                            'name_bn' => $pos->name_bn ?? '',
                            'candidates' => $pos->candidates->pluck('user_id')->map(function($id) { return (string)$id; })->values()->toArray()
                        ];
                    }
                } else {
                    $positionsData = [['id' => null, 'name' => '', 'name_bn' => '', 'candidates' => []]];
                }
                
                $optionsData = $election->type === 'poll' 
                    ? $election->pollOptions->pluck('text')->toArray() 
                    : ['', ''];
            @endphp
            return {
                type: '{{ $election->type }}',
                ...
                addPosition() {
                    this.positions.push({ id: null, name: '', name_bn: '', candidates: [] });
                },
